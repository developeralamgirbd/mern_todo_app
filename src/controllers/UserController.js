...
exports.register = async (req, res)=>{
	try{
		...
		await sendEmail(user.email, `Token will expire in ${expireTime} minute`, 'Todo App Account Verify', token);
...
exports.resendEmail = async (req, res)=>{
	try {
		...
		// Confirmation Token generated by user model
		const token = user.generateConfirmationToken();

		// Confirmation Token expire date
		let date = new Date();
		date.setMinutes( date.getMinutes() + 3);

		// Update confirmation token and date
		await UserService.confirmationTokenReset(user.email, token, date);
